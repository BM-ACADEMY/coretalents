...
const Testimonial = () => {
  ...
  const handleNext = () => {
    if (!loading) setCurrentIndex((prev) => (prev + 1) % testimonials.length);
  };
        ...
        <div className="flex flex-col items-start space-y-8">
          ...
          <div className="w-full">
            {loading ? (
              ...
              <>
                <h3 className="text-2xl font-bold text-slate-900 animate-fadeIn">
                  {currentData.name}
                </h3>
                <p className="text-slate-500 text-sm uppercase tracking-wide mt-1 animate-fadeIn">
                  {currentData.work}
                </p>
              </>
            )}
