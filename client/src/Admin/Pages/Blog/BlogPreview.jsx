            ...
            const renderBlock = (innerBlock, innerIndex) => {
                ...
                if (innerBlock.type === "paragraph") {
                  return (
                    <div
                      key={innerIndex}
                      className="text-gray-700 leading-7 whitespace-pre-line mb-3 [&>p]:mb-3 last:mb-0"
                      dangerouslySetInnerHTML={{
                        __html: innerBlock.data.text || "Paragraph text...",
                      }}
                    />
                  );
                }
                if (innerBlock.type === "image") {
                  return innerBlock.data.url ? (
                    <div key={innerIndex} className="my-4">
                      <img
                ...
                if (innerBlock.type === "list") {
                  return (
                    <div key={innerIndex} className="my-6">
                      ...
                      <ul className="grid grid-cols-1 gap-x-1 gap-y-4 bg-gray-50 p-6 rounded-2xl list-none">
                        {innerBlock.data.items.map((item, idx) => (
                          <li key={idx} className="flex items-start gap-3">
                            ...
                            {(innerBlock.data.listType === "bullet" ||
                ...
                if (innerBlock.type === "quote") {
                  return (
                    ...
                    >
                      "{innerBlock.data.text || "Quote text here..."}"
                ...
                if (innerBlock.type === "button") {
                  ...
                  const styleClass =
                    btnClasses[innerBlock.data.style] || btnClasses.primary;

                  return (
                    <div key={innerIndex} className="my-8 text-center">
                      <a
                        href={innerBlock.data.url || "#"}
                        target="_blank"
                      ...
                      >
                        {innerBlock.data.text || "Button Text"}{" "}
                        <ExternalLink size={18} />
                      </a>
                    </div>
                  );
                }
                if (innerBlock.type === "accordion") {
                  const isOpen = openAccordion === innerIndex;
                  return (
                    <div
                      key={innerIndex}
                      ...
                      {isOpen && (
                        <div className="p-6 bg-white border-t">
                          <p className="text-gray-700 leading-relaxed whitespace-pre-line">
                            {innerBlock.data.content || "Accordion content goes here..."}
                          </p>
                        </div>
                      )}
                    </div>
            ...
            if (block.type === 'section' && block.data && block.data.items) {
                return (
                    <div key={i} className="blog-section mb-14">
                        {block.data.items.map((colBlock, idx) => renderBlock(colBlock, `${i}-${idx}`))}
                    </div>
                );
            }
