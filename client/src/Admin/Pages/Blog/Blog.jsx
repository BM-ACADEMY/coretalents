...
const BlogCreate = ({ switchToView }) => {
  ...
  // --- Section State ---
  // A Section = { id: 1, isCompleted: false, items: [ {type:'heading', data:...}, ... ] }
  const [sections, setSections] = useState([
    { id: Date.now(), isCompleted: false, items: [] } // Start with 1 open section
  ]);
  
  const [submitting, setSubmitting] = useState(false);
  const [uploadingImg, setUploadingImg] = useState({ sectionIdx: null, itemIdx: null });

  // Check if the current active section has items
  const activeSectionIndex = sections.findIndex(s => !s.isCompleted);
  const isSectionEmpty = activeSectionIndex !== -1 && sections[activeSectionIndex].items.length === 0;
  ...
  // 1. Add a specific ITEM (Heading/Para/Image) to the Active Section
  const addItemToSection = (type) => {
    const activeIdx = sections.findIndex(s => !s.isCompleted);
    if (activeIdx === -1) return; // No active section

    const baseData = type === 'list' ? { items: [""] } : { text: "", url: "" };
    const newItem = { type, data: baseData };
  ...
  // 2. Add a Whole New Section (Only allowed if previous is done)
  const addNewSection = () => {
  ...
  // 3. Mark Section as Done
  const markSectionCompleted = (sectionIdx) => {
  ...
  // 4. Edit Section (Re-open)
  const editSection = (sectionIdx) => {
    // Close all others first to maintain focus
    const newSections = sections.map((s, i) => ({
      ...s,
      isCompleted: i !== sectionIdx // Only open the clicked one
    }));
    setSections(newSections);
  };
  ...
  const removeSection = (sectionIdx) => {
    ...
    // If we removed everything, add a fresh one
  ...
  const handleImageUpload = async (sIdx, iIdx, file) => {
    if (!file) return;
    setUploadingImg({ sectionIdx: sIdx, itemIdx: iIdx });
  ...
  const handleSubmit = async (e) => {
    ...
    // Flatten Sections into a single array of blocks for the backend
    ...
    formData.append("contentBlocks", JSON.stringify(flatBlocks)); // <--- Flattened here
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-150px)]">
      ...
      <div className="flex flex-col h-full overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="flex-grow space-y-6 pb-20"> 
              ...
              <div key={section.id} className={`transition-all duration-300 rounded-lg overflow-hidden ${section.isCompleted ? 'bg-white border border-gray-200 shadow-sm opacity-80 hover:opacity-100' : 'bg-white border-2 border-blue-500 shadow-xl ring-4 ring-blue-50'}`}>
                ...
                {!section.isCompleted && (
                  <div className="p-4 bg-gray-50">
                    ...
                    {/* List of items inside this section */}
                    <div className="space-y-4 mb-6">
                        {section.items.length === 0 && <div className="text-center text-gray-400 py-4 text-sm italic">Add items to this section using the buttons below</div>}
                            ...
                            <div key={iIdx} className="bg-white p-3 rounded shadow-sm border relative group animate-in slide-in-from-bottom-2 fade-in">
                                    ...
                                    <div>
                                        {item.data.url ? (
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <input type="file" onChange={(e)=>handleImageUpload(sIdx, iIdx, e.target.files[0])} className="text-xs text-gray-500"/>
                                ...
                                {item.type === 'list' && (
                                    <div className="space-y-1">
                                        ...
                                        <button onClick={()=>addListItem(sIdx, iIdx)} className="text-xs text-blue-600 font-bold flex items-center gap-1 mt-1"><Plus size={10}/> Add Item</button>
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    {/* INTERNAL TOOLBAR (To add items to this section) */}
                    <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                        <div className="flex gap-2">
                            ...
                            <SmallToolBtn icon={<ListIcon size={14}/>} label="List" onClick={()=>addItemToSection('list')}/>
                        </div>
                        <button 
                            onClick={() => markSectionCompleted(sIdx)} 
                            className="bg-black text-white px-5 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-gray-800 transition shadow-lg"
                        >
            ...
            {/* "Add New Section" Button (Only if no active section) */}
        ...
        {activeSectionIndex === -1 && (
            <div className="sticky bottom-4 bg-white/80 backdrop-blur p-4 border-t shadow-lg rounded-t-xl">
                 <button 
                  ...
                  disabled={submitting} 
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 rounded-lg font-bold shadow-md hover:shadow-xl transition transform hover:-translate-y-1 flex justify-center items-center gap-2"
                >
      ...
      <div className="hidden lg:block h-full bg-gray-100 rounded-xl overflow-hidden shadow-inner border relative">
         <div className="h-full overflow-y-auto p-8 custom-scrollbar">
             {/* We pass the sections array to preview, which will flatten it for display */}
